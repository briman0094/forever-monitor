  ...
  function startLogs(child, childData) {
    if (monitor.child) {
      ...
      if (!monitor.silent) {
        ...
        if (monitor.child.stdout) monitor.child.stdout.pipe(process.stdout, { end: false });
        if (monitor.child.stderr) monitor.child.stderr.pipe(process.stderr, { end: false });
      }

      if (monitor.stdout && monitor.child.stdout) {
        monitor.child.stdout.pipe(monitor.stdout, { end: false });
      }

      if (monitor.stderr && monitor.child.stderr) {
